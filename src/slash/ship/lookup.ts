...
export default
{
  ...
  run: async (client: Cleux, inter: CommandInteraction) => {
    ...
    database.findOne({ _id: 'ships' }, (_err: any, document: any) => {
      console.log(document[name!])
      const toTitleCase = function (string: string) { return string.valueOf().toLowerCase().replace(string.valueOf()[0], string.valueOf()[0].toUpperCase()) }
      ...
      if (!verifiedName || !otherName) return inter.reply({ content: 'Could not find that ship in the database.', ephemeral: true })
