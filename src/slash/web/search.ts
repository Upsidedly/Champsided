...
export default
{
  ...
  run: async (client: Cleux, inter: CommandInteraction) => {
    try {
      ...
      const results: any = await axios.get('https://customsearch.googleapis.com', {
        params: {
          q: query,
      ...
      if (!results.body.items) return inter.reply({ content: 'No results found.', ephemeral: true })
      if (results.status >= 400) return inter.reply({ content: 'Error.', ephemeral: true })
