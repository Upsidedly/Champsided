...
import discord, { Message, Util } from 'discord.js'
...
export default
{
  ...
  run: async (client: Cleux, message: Message, args: Array<string>, alias: string) => {
    if (!message.member?.permissions.has('MANAGE_EMOJIS_AND_STICKERS')) return message.reply({ content: 'You do not have the permission to use this command' })
    if (!args.length) return message.reply({ content: 'Please specify some emojis to steal.' })
    for (const rawmoji of args) {
      ...
      if (parsedEmoji!.id) {
        ...
        let emoji: discord.GuildEmoji | undefined
        try {
