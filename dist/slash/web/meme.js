...
export default {
    ...
    run: async (client, inter) => {
        let res = await axios.get('https://www.reddit.com/r/memes/random/.json');
        if (!res || !res.data || !res.data.length)
            return inter.reply({ content: 'An error occured.', ephemeral: true });
        ...
        const buttonRow = new discord.MessageActionRow().addComponents(new discord.MessageButton()
            .setStyle('SUCCESS')
            .setLabel('New Meme')
            .setCustomId('meme_new'), new discord.MessageButton()
            .setStyle('SECONDARY')
